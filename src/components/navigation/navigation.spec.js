...
describe('Components/Navigation', () => {
  ...
  describe('Nav Links', () => {
    ...
    it('should have the expected links in the correct order', () => {
      ...
      expect(navLinks[1]).to.equal('/tickets/');
      expect(navLinks[2]).to.equal('/faq/');
      expect(navLinks[3]).to.equal('/news/');
      expect(navLinks[4]).to.equal('/contact/');
    });
    ...
    it('should have the expected link labels in the correct order', () => {
      ...
      expect(navText[1]).to.equal('Tickets');
      expect(navText[2]).to.equal('FAQ');
      expect(navText[3]).to.equal('News');
      expect(navText[4]).to.equal('Contact');
    });
